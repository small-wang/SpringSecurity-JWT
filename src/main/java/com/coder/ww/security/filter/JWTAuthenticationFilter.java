package com.coder.ww.security.filter;

import com.coder.ww.exception.BusinessException;
import com.coder.ww.security.signature.ConstantKey;
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request) {
        ...
        try {
            ...
            if (user != null) {
                ...
                String[] split = roles.substring(1, roles.length() - 1).split(",");
                ArrayList<GrantedAuthority> authorities = new ArrayList<>();
                for (int i = 0; i < split.length; i++) {
                    if (!StringUtils.isEmpty(split[i])) {
                        authorities.add(new SimpleGrantedAuthority(split[i].trim()));
                    }
                }
